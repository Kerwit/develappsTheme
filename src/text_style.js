...
export function toDart(context, textStyle, depth = 1, useName = true) {
    ...
    if (textStyle.height != null) {
        ...
        if (!(skipDefaultValue && height == 1.0)) {
            if (!options.skipLineHeight(context)) {
                lineHeightElement = `\n${depthStr}height: ${textStyle.height} / ${textStyle.fontSize},`;
            }
        } 
    }
    ...
    return `GoogleFonts.${textStyle.fontFamily.charAt(0).toLowerCase() + textStyle.fontFamily.slice(1)}(
${depthStr}color: ${color.toDart(context, textStyle.color, 1)},${fontSizeElement}${fontWeightElement}${fontStyleElement}${letterSpacingElement}${lineHeightElement}${shadowElements}
${endDepthStr})`
}
