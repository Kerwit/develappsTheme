...
function layer(context, selectedLayer) {
    ...
    switch(selectedLayer.type) {
        ...
        case 'shape':
            ...
            break;
            case 'boxDecoration':
                body = decoration.widget(context, selectedLayer);
                break;   
        default:
            break;            
    }
...
function component(context, selectedVersion, selectedComponent) {
    const body =button.widget(context, selectedVersion, selectedComponent);
    return {
        code: body,
        language: "dart"
    }
...
function projectTextStylesToDart(context) {
   // const prefix = options.textStylesClassPrefix(context);
    ...
    context.project.textStyles.map(
        ts => {
            textStyles.push(`\tstatic final TextStyle ${util.camelize(ts.name)} = ${textStyle.toDart(context, ts, 2, false)};`)
        }
    );
